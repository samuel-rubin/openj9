        ...
        switch(params.JOB_TYPE) {
            ...
            case 'COUNT':
                if (params.JOB_TO_CHECK){
                    cleanupBuilds(ARTIFACTORY_SERVER_URL, ARTIFACTORY_REPO, JENKINS_DOMAIN, params.JOB_TO_CHECK, ARTIFACTORY_NUM_ARTIFACTS, artifactoryCreds)
                } else {
                    error 'Please input a job to cleanup'
                }
            break
            default:
                error 'the JOB_TO_CHECK parameter is empty. Please Enter TIME or COUNT'
            break
        }
    }
}

def cleanupBuilds(artifactory_server, artifactory_repo, jenkins_domain, jobToCheck, artifactory_num_artifacts, artifactoryCreds){
    ...
    stage('Discover Stored Artifacts'){
        ...
        def request = httpRequest authentication: artifactoryCreds, consoleLogResponseBody: true, url: "${artifactory_server}/api/storage/${artifactory_repo}/${jenkins_domain}/${jobToCheck}"
    ...
    stage('Delete Old Artifacts'){
        if (numberOfArtifacts > artifactory_max_num_artifacts){
            def folderNames = getFolderNumbers(data.children.uri)

            for(i=0; i  < (numberOfArtifacts - artifactory_max_num_artifacts); i++){
